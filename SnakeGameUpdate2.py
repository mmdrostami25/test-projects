...
class Game:
    ...
    def reset_game(self):
        ...
        self.snake2 = [[WIDTH/4, HEIGHT/4]]  # Second player
    ...
    def handle_input(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # Player 2 controls
                if event.key == pygame.K_a and self.direction2[0] != BLOCK_SIZE:
                    self.direction2 = [-BLOCK_SIZE, 0]
                elif event.key == pygame.K_d and self.direction2[0] != -BLOCK_SIZE:
                    self.direction2 = [BLOCK_SIZE, 0]
                elif event.key == pygame.K_w and self.direction2[1] != BLOCK_SIZE:
                    self.direction2 = [0, -BLOCK_SIZE]
                elif event.key == pygame.K_s and self.direction2[1] != -BLOCK_SIZE:
                    self.direction2 = [0, BLOCK_SIZE]

    def move(self):
        new_head = [self.snake[0][0] + self.direction[0], 
                   self.snake[0][1] + self.direction[1]]
        new_head2 = [self.snake2[0][0] + self.direction2[0],
                    self.snake2[0][1] + self.direction2[1]]
        
        new_head[0] = new_head[0] % WIDTH
        new_head[1] = new_head[1] % HEIGHT
        new_head2[0] = new_head2[0] % WIDTH
        new_head2[1] = new_head2[1] % HEIGHT
        
        self.snake.insert(0, new_head)
        self.snake2.insert(0, new_head2)
        
        if (abs(new_head[0] - self.apple[0]) < BLOCK_SIZE and 
            abs(new_head[1] - self.apple[1]) < BLOCK_SIZE):
    ...
    def check_collisions(self):
        ...
        for obs in self.obstacles:
            if (abs(self.snake[0][0] - obs["x"]) < BLOCK_SIZE and 
